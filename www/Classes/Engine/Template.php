...
class Template
{
  ...
  */
  var $last_error     = "";
    ...
    /**
     * Un tableau d'infos sur la page (title, description, keywords)
     * @var       array
     * @access    public
     * @see       halt
     */
    var $page_infos = array();
  
    /**
     * Un tableau de fichiers css à inclure
     * @var       array
     * @access    public
     * @see       halt
     */
    var $css_to_include = array();
    
    /**
     * Un tableau de fichiers javascript à inclure
     * @var       array
     * @access    public
     * @see       halt
     */
    var $js_to_include = array();
  ...
  * @return    boolean
  */
    function set_file($varname, $filename = "", $root = true) 
    {
        if (!is_array($varname)) 
        {
            if ($this->debug & 4) 
            ...
            $this->file[$varname] = $this->filename($filename, $root);
  ...
  * @see       set_root
  */
  function filename($filename, $root = true) {
    if ($this->debug & 4) {
  ...
  */
  function haltmsg($msg) {
    ...
    /******************************************************************************
    * Cette fonction ajoute des infos sur la page
    *
    * usage: add_page_infos($type, $name)
    *
    * @access    public
    * @return    none
    */
    
    function add_page_infos($type, $name)
    {
        $this->page_infos[$type] = $name;  
    }
    
    /******************************************************************************
    * Cette fonction ajoute des fichiers css à inclure
    *
    * usage: add_css($name, $type)
    *
    * @access    public
    * @return    none
    */
    
    function add_css($name, $type = 'all', $is_module = false)
    {
        if(!$is_module)
        {
            $this->css_to_include[$type][] = $name;
        }
        else
        {
            $this->css_to_include['module'][] = $name;
        }
    }
    ...
    */
    ...
    function load_css() 
    {   
        $CSS_PATH     = DOMAINE.ROOT.PATH_CSS;
        $CSS_PATH_MDL = DOMAINE.ROOT;

        if($this->https) {
            $CSS_PATH = str_replace("http", "https", $CSS_PATH);
            $CSS_PATH_MDL = str_replace("http", "https", $CSS_PATH_MDL);
        }
        
        define("CSS_PATH", $CSS_PATH);
        
        $this->set_file("CSS_FILES", PATH_TEMPLATE_COMMON."head-css".EXT_TPL);

        $this->set_block('CSS_FILES','list_all','var_list_all');
        
        // all the others
        if(isset($this->css_to_include['all'])) {   
            foreach($this->css_to_include['all'] as $CSS_FILE) {
                $this->set_var("CSS_FILE_ALL", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_all', 'list_all', true);
            }
        } else {
              $this->set_var("var_list_all","");
        }
        
        // IE
        $this->set_block('CSS_FILES','list_IE','var_list_IE');

        if(isset($this->css_to_include['ie'])) {
            foreach($this->css_to_include['ie'] as $CSS_FILE) {
                $this->set_var("CSS_FILE_IE", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_IE','list_IE',true);
            }
        } else {
              $this->set_var("var_list_IE","");
        }

        // IE 6
        $this->set_block('CSS_FILES','list_IE6','var_list_IE6');

        if(isset($this->css_to_include['ie6'])) {
            foreach($this->css_to_include['ie6'] as $value) {
                $this->set_var("CSS_FILE_IE6", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_IE6','list_IE6',true);
            }
        } else {
              $this->set_var("var_list_IE6","");
        }

        // IE 7
        $this->set_block('CSS_FILES','list_IE7','var_list_IE7');

        if(isset($this->css_to_include['ie7'])) {
            foreach($this->css_to_include['ie7'] as $value) {
              $this->set_var("CSS_FILE_IE7", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_IE7','list_IE7',true);
            }
        } else {
              $this->set_var("var_list_IE7","");
        }

        // IE 8
        $this->set_block('CSS_FILES','list_IE8','var_list_IE8');

        if(isset($this->css_to_include['ie8'])) {
            foreach($this->css_to_include['ie8'] as $value) {
                $this->set_var("CSS_FILE_IE8", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_IE8','list_IE8',true);
            }
        } else {
              $this->set_var("var_list_IE8","");
        }

        // IE 9
        $this->set_block('CSS_FILES','list_IE9','var_list_IE9');

        if(isset($this->css_to_include['ie9'])) {
            foreach($this->css_to_include['ie9'] as $value) {
                $this->set_var("CSS_FILE_IE9", CSS_PATH.$CSS_FILE.'.css?v='.VERSION);
                $this->parse('var_list_IE9','list_IE9',true);
            }
        } else {
              $this->set_var("var_list_IE9","");
        }
        
        // Modules
        $this->set_block('CSS_FILES','list_MDL','var_list_MDL');

        if(isset($this->css_to_include['module'])) {
            foreach($this->css_to_include['module'] as $MODULE_NAME) {
                $this->set_var("CSS_FILE_MDL", $CSS_PATH_MDL.PATH_TEMPLATE_MODULE.$MODULE_NAME.'/style.css?v='.VERSION);
                $this->parse('var_list_MDL','list_MDL',true);
            }
        } else {
              $this->set_var("var_list_MDL","");
        }
    }
  
    /******************************************************************************
    * Cette fonction injecte du javascript
    *
    * usage: add_js($name, $pos = 'footer', $type = 'file')
    *
    * @var string $string nom de fichier ou code à insérer   
    * @var string $pos positionnement de l'insertion dans la page (<head> ou footer)
    * @var string $type type d'insertion 
    * -> file (récupérer le contenu d'un fichier) 
    * -> code (insertion directe du code - dynamique)
    * -> path (insertion par src)    
    *    
    * @access    public
    * @return    none
    */
      
    function add_js($file, $pos = 'header', $is_module=false)
    {
        if(!$is_module) {
            $this->js_to_include[$pos][] = $file;
        } else {
            $this->js_to_include['module'][] = $file;
        }
    ...
    /******************************************************************************
    * Charge les fichiers javascripts
    *
    * usage: load_javascript() 
    *
    * @access    public
    * @return    none
    */
    
    function load_javascript($where) 
    {
        // Module
        if($where=='module') {   
            if(isset($this->js_to_include['module'])) {
                foreach($this->js_to_include['module'] as $MODULE_NAME) {                 
                    $this->set_var("JS_FILE", DOMAINE.ROOT.PATH_TEMPLATE_MODULE.$MODULE_NAME.'/js.js?v='.VERSION);
                    $this->parse('var_list_js_header', 'list_js_header',true);
                }
            }
        ...
        $JS_PATH = DOMAINE.ROOT.'/'.PATH_JS;

        if($this->https) {
            $JS_PATH = str_replace("http", "https", $JS_PATH);
        }
     
        $this->set_file("JS_FILES_".$where, PATH_TEMPLATE_COMMON.$where."-js".EXT_TPL);
        
        $this->set_var("DOMAINE", DOMAINE);
        $this->set_var("ROOT",    ROOT);

        $this->set_block('JS_FILES_'.$where, 'list_js_'.$where, 'var_list_js_'.$where);

        if(isset($this->js_to_include[$where])) {         
            foreach($this->js_to_include[$where] as $JS_FILE) {   
                $this->set_var("JS_FILE", $JS_PATH.$JS_FILE.'.js?v='.VERSION);
                $this->parse('var_list_js_'.$where, 'list_js_'.$where,true);
            }
        } else {
            $this->set_var('var_list_js_'.$where, '');
        }
    }  
 
    /******************************************************************************
    * Cette fonction permet l'insertion de module
    *
    * usage: load_Module($nameFile, $moduleName, $moduleVar, $arrVariables = '')
    *
    * @access    public
    * @return    none
    */  
  
    function load_module($MDL_name, $MDL_tpl_var, $param = '', $file_name_TPL = '')
    {
        if($file_name_TPL=='')
            $file_name_TPL = $MDL_name.'_'.mt_rand(100, 999).'_'.time();

        $this->set_file( $file_name_TPL, PATH_TEMPLATE_MODULE.$MDL_name.'/template'.EXT_TPL, false );
    
        if(file_exists(PATH_TEMPLATE_MODULE.$MDL_name.'/style.css'))
            $this->add_css($MDL_name, 'all', true);
        
        if(file_exists(PATH_TEMPLATE_MODULE.$MDL_name.'/js.js'))
            $this->add_js($MDL_name, 'module', true);
        

        include PATH_TEMPLATE_MODULE.$MDL_name.'/core.php';
        

        $this->set_var($MDL_tpl_var, $this->parse('module_display', $file_name_TPL));
    
        return 1;
    }

    /******************************************************************************
    * Cette fonction permet l'insertion de flash
    *
    * usage: flash($varname, $url, $width, $height, $alt, $flashvars = '', $params = '', $attributes = '', $version = '9.0.0') 
    *
    * @access    public
    * @return    none
    */
    
    function flash($varname, $url, $width, $height, $alt, $flashvars = '', $params = '', $attributes = '', $version = '9.0.0') 
    {
        $idContent = $varname.mt_rand(0,100);        
        $experssInstallUrl = PATH_SWF.'expressInstall.swf';
        
        $flashvarsContent = 'var flashvars = {}
            ';      
        if(is_array($flashvars))
        {       
            foreach($flashvars as $key => $value)
            {
                $flashvarsContent .= 'flashvars.'.$key.' = "'.$value.'";
            ';                
            }
        }        
        
        
        $paramsContent = 'var params = {';       
        if(is_array($params))
        {
        ...
        $attributesContent = 'var attributes = {';  
        ...
        $this->add_js_code (
            '           // Animation flash '.$url.'.swf
            '.$flashvarsContent.'
            '.$paramsContent.'
            '.$attributesContent.'
            swfobject.embedSWF("'.PATH_SWF.$url.'.swf", "'.$idContent.'", "'.$width.'", "'.$height.'", "'.$version.'", "'.$experssInstallUrl.'", flashvars, params, attributes);', 'head');
        
        
        $this->set_var($varname, $content); 
    }
  
    function display_page($header = "header-default", $footer = "footer-default")
    {
        global $AUTH;
    
        $footer = PATH_TEMPLATE_COMMON.PATH_TEMPLATE_FOOTER.$footer.EXT_TPL;
        $header = PATH_TEMPLATE_COMMON.PATH_TEMPLATE_HEADER.$header.EXT_TPL;
                
        /**
         * CHARGEMENT DES COMPOSANTS DU <head>
         **/
        
        // title, description keywords
        $this->set_file("file_page_infos", PATH_TEMPLATE_COMMON."head-page-infos".EXT_TPL);
        
        $this->set_var("TITLE",        $this->page_infos['title']);
        $this->set_var("DESCRIPTION",  $this->page_infos['description']);
        $this->set_var("KEYWORDS",     $this->page_infos['keywords']);
        $this->set_var("SITE_TITLE",   SITE_TITLE);
        
        $this->set_file("file_metas", PATH_TEMPLATE_COMMON."head-metas".EXT_TPL);

        $this->set_var("AUTHOR", AUTHOR);
        $this->set_var("ROOT", ROOT);

        $this->load_css();
        $this->load_javascript('header');
        $this->load_javascript('module');

        $this->set_file("PAGE_BEGINNNING", PATH_TEMPLATE_COMMON."page-beginning".EXT_TPL);
        
        $this->set_var('INFOS_PAGE',    $this->parse("page_infos_display","file_page_infos"));
        $this->set_var('META_TAGS',     $this->parse("meta_display","file_metas"));
        $this->set_var('FILES_CSS',     $this->parse("display_css","CSS_FILES"));
        $this->set_var('FILES_JS_HEAD', $this->parse("display_js_head","JS_FILES_header"));

        $this->set_file('HEADER', $header);
        
        /*if(!preg_match('/landing-page/', BASENAME))
        {
            if($AUTH->get_value('connected')==true)
            {
                $this->set_block('HEADER','USER_NOT_CONNECTED', '$USER_NOT_CONNECTED');
                $this->set_var('$USER_NOT_CONNECTED', '');
                $this->set_var('ACCOUNT_NAME', $_SESSION['user_name']);
            }
            else
            {   
                $this->set_block('HEADER', 'USER_CONNECTED', '$USER_CONNECTED');
                $this->set_var('$USER_CONNECTED', '');
            }
        }*/
        ...
        $isCurrentPage = false;
        $currentPage = $this->page_infos['categorie'];

        //$this->set_var('PAGE_ACTUELLE',$currentPage);
        //$this->set_var('ITF_current_page',$currentPage);

        $this->set_file('FOOTER', $footer);
        $this->set_file("PAGE_END",PATH_TEMPLATE_COMMON."page-end".EXT_TPL);

        $this->set_var("ANALYTICS_CODE", ANALYTICS_CODE);

        $this->load_javascript('footer');
        
        $this->set_var('FILES_JS', $this->parse("display_js","JS_FILES_footer"));
        
        echo $this->parse('displayStartSite','PAGE_BEGINNNING');
        echo $this->parse('displayHeader',   'HEADER');
        echo $this->parse('displayBody',     'BODY');
        echo $this->parse('displayFooter',   'FOOTER');
        echo $this->parse('displayEndSite',  'PAGE_END');
    }
